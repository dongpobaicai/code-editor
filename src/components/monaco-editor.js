...
import "monaco-editor/esm/vs/basic-languages/html/html.contribution";  // 引入编辑器html格式文件
import "monaco-editor/esm/vs/basic-languages/scss/scss.contribution"; // 引入编辑器css格式文件
...
function inlineShortArraysInResult(result, width) {
  ...
  if (typeof width != "number" || width < 20) {
    throw (
      "Invalid width '" + width + "'. Expecting number equal or larger than 20."
    );
  }
...
export default {
  ...
  render() {
    return <div class="monaco_editor_container" style={this.style}></div>;
  },
